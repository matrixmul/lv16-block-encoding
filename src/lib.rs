...
pub fn render_baseline_qasm(target: &Value) -> String {
    let target_qubits = target["qubits"].as_u64().expect("target qubits") as usize;
    ...
    let mut lines = vec![
        ...
        format!("// declared_width: {target_qubits}"),
        format!(
            "// terms_sha256: {}",
    ...
    for term in terms {
        ...
        let term_qubits = qubit_values
            .iter()
            .map(|value| value.as_u64().expect("qubit") as usize)
            .collect::<Vec<_>>();
        assert!(
            term_qubits.iter().all(|&q| q < target_qubits),
            "term qubits exceed target width"
        );
        let angle = qasm_angle(term["angle"].as_f64().expect("term angle"));
        match kind {
            "z_phase" => lines.push(format!("rz({angle}) q[{}];", term_qubits[0])),
            "zz_phase" => {
                lines.push(format!("cx q[{}], q[{}];", term_qubits[0], term_qubits[1]));
                lines.push(format!("rz({angle}) q[{}];", term_qubits[1]));
                lines.push(format!("cx q[{}], q[{}];", term_qubits[0], term_qubits[1]));
            }
            "x_mixer" => {
                lines.push(format!("h q[{}];", term_qubits[0]));
                lines.push(format!("rz({angle}) q[{}];", term_qubits[0]));
                lines.push(format!("h q[{}];", term_qubits[0]));
            }
