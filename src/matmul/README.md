# MatrixMul LV16 35q Same-Width Submission Note
...
42-qubit baseline; it declares and implements a real 35-qubit circuit.
...
2. For each of the four MatrixMul rounds, apply deterministic `same_width/z`
   `rz` phases on all 35 wires.
3. Apply nearest-neighbor `same_width/matrix_edge` parity gadgets across the
   34 adjacent pairs: `cx q[i], q[i+1]; rz(angle) q[i+1]; cx q[i], q[i+1];`.
...
Compared with the accepted 36q same-width submission, the 35q route removes one
